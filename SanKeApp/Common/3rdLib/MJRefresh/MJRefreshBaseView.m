...
//    
    ...
    if (_scrollView.isDragging) {
        ...
        _imageView.transform = CGAffineTransformMakeRotation(2*M_PI*r);
        if (_state == MJRefreshStateNormal) {
            CGFloat f = (MIN(offsetY, validOffsetY) / validOffsetY);
            _imageView.transform = CGAffineTransformMakeScale(f, f);
            _imageView.transform = CGAffineTransformRotate(_imageView.transform, 2*M_PI*r);
            _imageView.alpha = f;
...
- (void)observeValueForKeyPath2:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    ...
    //    if (_scrollView.isDragging) {
    //        CGFloat validOffsetY = validY + MJRefreshViewHeight;
    //        if (_state == MJRefreshStateNormal && offsetY > validOffsetY) {
    //            // 转为即将刷新状态
    //            [self setState:MJRefreshStateRefreshing];
    //            // 通知代理
    //            if ([_delegate respondsToSelector:@selector(refreshView:stateChange:)]) {
